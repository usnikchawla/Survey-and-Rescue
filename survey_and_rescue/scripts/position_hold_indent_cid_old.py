...
class Edrone():
	...
	def __init__(self):
		...
		self.iter = 0 #helps to make sure that the drone is within the specifed error limits at a point
		self.pos = 0 #helps to update self.setpoint
		self.translation = []
		#(3.2
		with open("/home/nitish/catkin_ws/src/survey_and_rescue/scripts/square_coords.json", "r") as read_file:
			self.cords = json.load(read_file)

		self.cords_keys_sorted = sorted(self.cords)
		self.i = 0
		for key in self.cords_keys_sorted:
			self.translation.append(self.cords[key])
			self.i += 1
			if self.i >= len(self.cords):
				break
				#3.2)
		print(self.cords)
		print(self.cords_keys_sorted)
		...
		self.Kp = [31.44,31.44,32.76] #for fast respone
		self.Ki = [0,0,0]
		self.Kd = [570,576.3,576.3]
		...
		#self.min_values = [1450,1450,1460]
	...
	#	8. Add error_sum
		#print("arrived in pid")
		curr = self.translation[self.pos] #gets the new target position
		self.setpoint = [curr[0],curr[1],curr[2]]
		...
		#(3.2
		if abs(self.error[0]) < 0.5 and abs(self.error[1]) < 0.5 and abs(self.error[2]) < 1.0:
		#3.2) 
		# checks if the error is in specified limits
			self.iter+=1 # we increase the iter until a specified count to make sures that the drone in its steady state at a point is within the error limits
			print(self.iter)
		if self.iter == 10:# after the drone is within the specified error limits at a point we give it the next target location but if it is the last point we land it there.  
			self.pos+=1
			rospy.sleep(3)
			if self.pos == len(self.translation): #What does this do usnik? #3.2
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		#if e_drone.pos != len(e_drone.translation):	#double check error? #3.2
		e_drone.pid()
		#(3.2
		if e_drone.pos == len(e_drone.translation):     
			e_drone.disarm()
			break
		#3.2)
		r.sleep()